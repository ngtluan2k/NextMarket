import {
  ...
  export class VouchersService {
    ...
    async getCollectedPlatformVouchers(userId: number): Promise<ExtendedVoucher[]> {
      ...
      const collectedVouchers = userCollections.map(c => c.voucher);
      ...
      const extended: ExtendedVoucher[] = collectedVouchers.map(voucher => ({
        ...voucher,
    ...
    async getCollectablePlatformVouchers(userId: number): Promise<ExtendedVoucher[]> {
      ...
      const collectedVoucherIds = new Set(userCollections.map(c => c.voucher.id));
      ...
      const collectableVouchers: ExtendedVoucher[] = [];

      for (const voucher of allPlatformVouchers) {
        if (collectedVoucherIds.has(voucher.id) && voucher.collection_type !== VoucherCollectionType.AUTO) continue;

        if (voucher.total_usage_limit && voucher.total_used_count >= voucher.total_usage_limit) continue;

        if (voucher.collection_limit && voucher.collected_count >= voucher.collection_limit) continue;
        ...
        if (voucher.new_user_only) {
          ...
          if (userOrders > 0) continue;
        }

        if (voucher.applicable_user_ids?.length && !voucher.applicable_user_ids.includes(userId)) continue;

        collectableVouchers.push({
          ...voucher,
      ...
      return collectableVouchers.sort((a, b) => {
        return new Date(a.end_date).getTime() - new Date(b.end_date).getTime();
      });
    }
  }