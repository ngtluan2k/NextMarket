...
export const userVoucherApi = {
   ...
   */
  getCollectedVouchers: async (): Promise<Voucher[]> => {
    const res = await api.get(`${ENDPOINTS.userVouchers}/collected`);
    return res.data;
  },

  /**
   * Get vouchers user can collect
   * @returns Array of collectable vouchers
   */
  getCollectableVouchers: async (): Promise<Voucher[]> => {
    const res = await api.get(`${ENDPOINTS.userVouchers}/available`);
    return res.data;
  },

  /**
   * Get usable vouchers for user
   * @param storeId - Optional store ID to filter vouchers
   * @param filterByStoreOnly - If true, exclude admin vouchers (default: false)
   * @returns Array of usable vouchers
   */
  getUsableVouchers: async (
    storeId?: number,
    ...
    const url = queryString 
      ? `${ENDPOINTS.userVouchers}/usable?${queryString}`
      : `${ENDPOINTS.userVouchers}/usable`;
   ...
   * @deprecated Use getUsableVouchers instead
   */
  getAvailableVouchersByStore: async (
    storeId: number,
    includeAdminVouchers = true
  ): Promise<Voucher[]> => {
    return userVoucherApi.getUsableVouchers(storeId, !includeAdminVouchers);
  },
