...
import { Spin, Card, Typography, Row, Col, Image, Tag, Button, message } from 'antd'; // Thêm Button và message
import {
...
import { userVoucherApi } from '../../api/voucher.api'; // 👈 Thêm import API voucher (điều chỉnh path nếu cần)
...
export default function PublicCampaignPage({ campaignId }: Props) {
  ...
  const [collectedVouchers, setCollectedVouchers] = useState<Set<number>>(new Set()); // 👈 State theo dõi voucher đã collect
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // 👈 Optional: Nếu user logged in, fetch my-vouchers để check collected
  ...
  // Hàm handle collect voucher 👈 Thêm hàm này
  const handleCollectVoucher = async (voucherId: number) => {
    ...
    try {
      ...
      setCollectedVouchers(prev => new Set([...prev, voucherId])); // Cập nhật state
    } catch (err) {
      message.error('Lỗi khi thu thập voucher: ' + (err as Error).message);
    }
  };
