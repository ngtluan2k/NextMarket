...
const VoucherDiscountSection: React.FC<Props> = ({
  ...
  const fetchAvailableVouchers = async () => {
    ...
    try {
      ...
      // Fetch my vouchers (collected)
      const my = await userVoucherApi.getMyVouchers();
      setMyVouchers(my);

      // Extend available vouchers with is_collected
      const extendedAllVouchers: ExtendedVoucher[] = allVouchers.map(v => ({
        ...v,
        is_collected: my.some(m => m.id === v.id),
        user_used_count: my.find(m => m.id === v.id)?.user_used_count || 0, // Assuming backend provides or calculate
      }));

      setApplicableVouchers(extendedAllVouchers);

      // Merge with selected and cache, extend them too
      const extendedSelected = selectedVouchers.map(v => ({
        ...v,
        is_collected: my.some(m => m.id === v.id),
        user_used_count: my.find(m => m.id === v.id)?.user_used_count || 0,
      })) as ExtendedVoucher[];
      ...
      // Set expired from my vouchers
      const expired = my.filter(v => 
        new Date() > new Date(v.end_date) || v.status === VoucherStatus.EXPIRED
      ).map(v => ({
        ...v,
        is_collected: true, // Since from myVouchers
 ...
 const isVoucherDisabled = (voucher: ExtendedVoucher) => {
  ...
  // For MANUAL, disable if not collected
  if (voucher.collection_type === VoucherCollectionType.MANUAL && !voucher.is_collected) {
    return true;
  }
  ...
  // For MANUAL, check collected
  if (voucher.collection_type === VoucherCollectionType.MANUAL && !voucher.is_collected) {
    message.warning('Vui lòng thu thập voucher trước khi chọn');
    return;
  }
  ...
  const handleApplyCode = async () => {
    ...
    try {
      ...
      const extendedVoucher: ExtendedVoucher = {
        ...voucher,
        is_collected: myVouchers.some(m => m.id === voucher.id),
