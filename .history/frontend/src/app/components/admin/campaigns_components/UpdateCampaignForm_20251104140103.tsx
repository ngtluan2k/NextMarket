...
export default function UpdateCampaignForm({ campaignId, onClose }: Props) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const imagesPayload = bannerFiles.filter(f => f.originFileObj).map((f) => ({
      ...
      // Sửa lỗi: Update mỗi voucher để set collection_type = 'EVENT'
      for (const vId of selectedVouchers) {
        await voucherApi.updateVoucher(vId, { collection_type: VoucherCollectionType.EVENT, applicable_user_ids: [] });
      }
