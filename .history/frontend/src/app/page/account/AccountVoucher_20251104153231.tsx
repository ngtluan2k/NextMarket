...
const AccountVoucher: React.FC = () => {
  ...
  const fetchUserVouchers = async () => {
    ...
    setLoading(true);
    try {
      const [collected, collectable] = await Promise.all([
        userVoucherApi.getCollectedVouchers(),
        userVoucherApi.getCollectableVouchers()
      ]);
      setVouchers([...collected, ...collectable]);
      console.log('📦 Loaded user platform vouchers:', collected.length + collectable.length);
    } catch (error: any) {
  ...
  const getFilteredVouchers = () => {
    ...
    switch (activeTab) {
      case 'available':
        //  Voucher có thể sử dụng: active + còn lượt
      ...
      case 'used':
        //  Voucher đã sử dụng hết hoặc không còn active
        filtered = filtered.filter((voucher) => {
          const userUsed = voucher.user_used_count || 0;
          return (
            userUsed > 0 &&
      ...
      case 'expired':
        //  Voucher hết hạn và chưa dùng
        filtered = filtered.filter((voucher) => {
          const userUsed = voucher.user_used_count || 0;
          return isVoucherExpired(voucher) && userUsed === 0;
        });
        break;
    }
    ...
    <div style={{ padding: '24px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      >
        <Tabs activeKey={activeTab} onChange={setActiveTab}>
          <TabPane
            tab={
              <Badge count={getFilteredVouchers().length} offset={[10, 0]} overflowCount={99}>
                <span>
                  <CheckCircleOutlined />
                  Có thể sử dụng
                </span>
    ...
    >
                  ...
                  <>
                    {!voucher.is_collected &&
                      [
                        ...
                        VoucherCollectionType.EVENT,
                      ].includes(voucher.collection_type) && (
                        <Button
                          type="primary"
                          size="small"
      ...
      >
        <div style={{ lineHeight: '2' }}>
          ...
          <div>
            <strong>Loại:</strong> <Tag color="blue">Voucher Sàn</Tag>
          </div>
          <div>
