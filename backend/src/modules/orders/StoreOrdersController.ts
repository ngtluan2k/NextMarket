...
export class StoreOrdersController {
  ...
  async findByStore(
    ...
    if (!req.user?.isStoreOwner && !req.user?.roles?.includes('admin')) {
  ...
  async getStoreRevenue(
    ...
    if (!req.user?.isStoreOwner  && !req.user?.roles?.includes('admin')) {
      throw new ForbiddenException('Bạn không có quyền xem doanh thu cửa hàng này');
    }
  ...
  async getStoreStats(
    ...
    if (!req.user?.isStoreOwner  && !req.user?.roles?.includes('admin')) {
      throw new ForbiddenException('Bạn không có quyền xem thống kê cửa hàng này');
    }
