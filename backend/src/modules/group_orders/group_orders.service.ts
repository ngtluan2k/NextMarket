...
export class GroupOrdersService {
  ...
  async handleExpiredGroups() {
    ...
    for (const group of openGroups) {
      ...
      if (remainingMembers < 2) {
        ...
        await this.gateway.broadcastGroupUpdate(
          ...
          'group-cancelled-timeout',
          {
            groupId: group.id,
            reason:
              'Không đủ thành viên sau khi loại bỏ người chưa chọn sản phẩm',
            remainingMembers,
          }
        );
        continue;
      ...
      this.logger.log(
        ` Group #${
          group.id
        } auto-locked, next expires_at=${nextExpires.toISOString()}`
      );
    }
      ...
      for (const g of lockedGroups) {
        ...
        await this.gateway.broadcastGroupUpdate(
          ...
          'group-cancelled-timeout',
          {
            groupId: g.id,
            message:
              '⏰ Nhóm đã bị hủy vì quá 30 phút sau khi khóa mà không hoàn tất.',
          }
        );
      }

      this.logger.log(
        `❌ Auto-cancelled ${lockedGroups.length} locked groups by timeout`
      );
    }
  }
  ...
  async createGroupOrder(dto: CreateGroupOrderDto) {
    ...
    if (dto.affiliateCode) {
      try {
        ...
        const affiliateInfo =
          await this.affiliateResolutionService.resolveAffiliateCode(
            dto.affiliateCode
          );
        ...
        if (affiliateInfo && affiliateInfo.isValid) {
          ...
          console.log(
            `✅ Resolved group affiliate: userId=${groupAffiliateUserId}, programId=${groupAffiliateProgramId}`
          );
        } else {
          console.warn(
            `⚠️ Could not resolve group affiliate code: ${dto.affiliateCode}`
          );
        }
      } catch (error) {
        console.error(
          `❌ Error resolving group affiliate code ${dto.affiliateCode}:`,
          error
        );
      }
    }

    const joinExpiresAt = dto.joinExpiresAt
      ? new Date(dto.joinExpiresAt)
      : null;
    ...
    const savedGroup = Array.isArray(saved) ? saved[0] : saved;
    return this.getGroupOrderById(savedGroup.id, dto.hostUserId);
  }
  ...
  async joinGroupOrder(
    ...
    if (
      ...
      group.join_expires_at.getTime() <= Date.now()
    ) {
    ...
    if (
      joinCode !== undefined &&
      group.join_code &&
      group.join_code !== joinCode.trim().toUpperCase()
    ) {
    ...
    const affiliateLinkId: number | null = null;
    ...
    if (affiliateCode) {
      try {
        ...
        if (affiliateInfo && affiliateInfo.isValid) {
          ...
          console.log(
            `✅ Resolved affiliate: userId=${affiliateUserId}, programId=${affiliateProgramId}`
          );
        } else {
        ...
        console.error(
          `❌ Error resolving affiliate code ${affiliateCode}:`,
          error
        );
    ...
    console.log(
      `✅ Member ${userId} joined group ${groupId} with affiliate tracking:`,
      {
        affiliateCode,
        affiliateUserId,
        affiliateProgramId,
  ...
  async joinGroupOrderByJoinCode(
    joinCode: string,
    userId: number,
    affiliateCode?: string
  ) {
  ...
  async updateGroupOrder(
    ...
    if (
      dto.delivery_mode &&
      ['host_address', 'member_address'].includes(dto.delivery_mode)
    ) {
    ...
    if (typeof dto.targetMemberCount === 'number') {
      ...
      if (group.status !== 'open') {
        throw new BadRequestException(
          'Không thể sửa mục tiêu khi nhóm đã khóa'
        );
      }
  ...
  async checkoutGroupOrder(
    ...
    if (group.status !== 'locked') {
      throw new BadRequestException('Vui lòng khóa nhóm trước khi thanh toán!');
    }
    ...
    console.log(
      ` Group #${groupId} completed by host payment (host_address mode)`
    );
  ...
  private async checkoutHostAddress(
    ...
    if (voucherCode && voucherCode.trim()) {
      try {
        ...
        if (
          ...
          throw new BadRequestException(
            `Mua nhóm chỉ được áp dụng voucher PLATFORM hoặc STORE. Voucher này là loại ${this.getVoucherTypeName(
              appliedVoucher.type
            )}.`
          );
        }
        ...
        this.logger.log(
          `Voucher ${voucherCode} (${this.getVoucherTypeName(
            appliedVoucher.type
          )}) validated - Discount: ${discountTotal}đ`
        );
      } catch (err: any) {
    ...
    const memberWithAffiliate = members.find(
      (m) => m.referrer_affiliate_user_id
    );
    if (memberWithAffiliate) {
      ...
      affiliateProgramId =
        memberWithAffiliate.referrer_affiliate_program_id || null;
      affiliateLinkId = memberWithAffiliate.referrer_affiliate_link_id || null;
      console.log(
        `✅ Using member-specific affiliate: code=${affiliateCode}, userId=${affiliateUserId}`
      );
    }
    // Priority 2: Check group-level affiliate (host inheritance)
      ...
      console.log(
        `✅ Using group-level affiliate: code=${affiliateCode}, userId=${affiliateUserId}`
      );
    } else {
    ...
    console.log(
      `✅ Order ${(savedOrder as any).id} created with affiliate tracking:`,
      {
        affiliateCode,
        affiliateUserId,
        affiliateProgramId,
  ...
  private async checkoutMemberAddresses(
    ...
    for (const [memberId, memberItems] of itemsByMember.entries()) {
      ...
      if (!member || !member.address_id) {
        throw new BadRequestException(`Member #${memberId} không có địa chỉ`);
      }
  ...
  async updateOrderStatus(
    ...
    return {
      success: true,
      message: `Order status updated: ${this.getOrderStatusText(
        oldStatus
      )} → ${this.getOrderStatusText(orderStatus)}`,
      data: {
        groupOrderId,
  ...
  async updateOrderStatusWithOrders(
    ...
    if (orderIds.length > 0) {
      ...
      const historyNote =
        note || `Cập nhật hàng loạt từ group order #${groupOrderId}`;
  ...
  async leaveGroupOrder(groupId: number, userId: number) {
    ...
    if (group.status !== 'open') {
      throw new BadRequestException(
        'Không thể rời nhóm khi nhóm đã bị khóa hoặc đã hoàn thành'
      );
    }
    ...
    if (group.user.id === userId) {
      throw new BadRequestException(
        'Chủ nhóm không thể rời nhóm. Vui lòng xóa nhóm nếu muốn hủy.'
      );
    }
    ...
    if (memberItems.length > 0) {
      await this.groupOrderItemRepo.delete(memberItems.map((item) => item.id));
  ...
  async checkoutMemberItems(
    ...
    if (voucherCode && voucherCode.trim() && group.user.id === userId) {
      try {
        ...
        this.logger.log(
          ` Host voucher: Total ${totalDiscount}đ, Member ${userId} ratio ${(
            ratio * 100
          ).toFixed(2)}%, discount ${discountTotal}đ`
        );
    ...
    return {
      ...
      voucherDiscount:
        discountTotal > 0
          ...
          : null,
      redirectUrl:
        typeof payment === 'object' && 'redirectUrl' in payment
          ? payment.redirectUrl
          : null,
    };
  }
