...
export class StoreFollowersService {
  ...
  private async assertStore(storeId: number) {
    const store = await this.storeRepo.findOne({ where: { id: storeId, is_deleted: false } });
    if (!store) throw new NotFoundException('Store not found');
  }

  async follow(userId: number, storeId: number) {
    await this.assertStore(storeId);
    const exists = await this.followerRepo.findOne({ where: { user_id: userId, store_id: storeId } });
    if (exists) return { followed: true };
    const row = this.followerRepo.create({ user_id: userId, store_id: storeId });
  ...
  async toggle(userId: number, storeId: number) {
    ...
    return status.followed ? this.unfollow(userId, storeId) : this.follow(userId, storeId);
  }
  ...
  async count(storeId: number) {
    await this.assertStore(storeId);
    const count = await this.followerRepo.count({ where: { store_id: storeId } });
    return { count };
  }
}