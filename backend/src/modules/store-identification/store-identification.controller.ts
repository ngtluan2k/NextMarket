...
export class StoreIdentificationController {
  constructor(private readonly storeIdentificationService: StoreIdentificationService) {}
  ...
  async uploadTempImage(
    ...
    const result = await this.storeIdentificationService.uploadTempImage(file, side);
  ...
  @ApiResponse({ status: 200, description: 'Front image uploaded successfully' })
  @UseInterceptors(FileInterceptor('file', multerConfig))
  ...
  @ApiResponse({ status: 200, description: 'Store identification updated successfully' })
  async update(
    @Param('storeId', ParseIntPipe) storeId: number,
