...
export class VnpayStrategy implements PaymentStrategy {
  ...
  async createPayment(order: any, paymentMethod: any, isGroup: boolean = false) {
  ...
  public async handleCallback(payload: any, rawQuery?: string): Promise<any> {
    ...
    if (!payment)
      throw new Error(`Payment not found for TxnRef: ${payload.vnp_TxnRef}`);
    ...
    try {
      ...
      Object.keys(payload).forEach((key) => {
        if (key !== 'vnp_SecureHash' && key !== 'vnp_SecureHashType') {
          let value = Array.isArray(payload[key])
            ? payload[key][0]
            : payload[key];
          value = String(value);
          params[key] = value;
      ...
      return (
        computedSignature.toLowerCase() === String(vnp_SecureHash).toLowerCase()
      );
    } catch (err) {
      this.logger.error(
  ...
  public verifySignatureFromQuery(payload: any, rawQuery?: string): boolean {
    try {
      ...
      console.log('Secret Key:', vnp_HashSecret);
      console.log('Received Hash:', vnp_SecureHash);

      if (!vnp_SecureHash) return false;

      // Chuẩn bị params từ rawQuery để giữ nguyên định dạng
      const params: Record<string, string> = {};
      if (rawQuery) {
      ...
      // Sắp xếp và tạo sign data, giữ nguyên ký tự đặc biệt
      const sortedParams = this.sortObject(params);
      const signData = Object.keys(sortedParams)
        .map(
          (k) =>
            `${k}=${encodeURIComponent(sortedParams[k]).replace(/%20/g, '+')}`
        )
        .join('&');
  ...
  private async findPaymentByTxnRef(txnRef: string): Promise<Payment | null> {
    ...
    return await this.paymentRepo.findOne({
      where: [
        { uuid: normalized },
        { uuid: this.formatToUuid(normalized) || '' },
      ],
      relations: ['order'],
    });
  }
  ...
  private formatToUuid(txnRef: string): string | null {
    if (txnRef.length === 32) {
      return `${txnRef.substring(0, 8)}-${txnRef.substring(
        8,
        12
      )}-${txnRef.substring(12, 16)}-${txnRef.substring(
        16,
        20
