...
export class AffiliateLinksService {
  ...
  /**
   * Create Group Affiliate Link - Host-Based Affiliate for Group Buying
   * Host tạo group buying với affiliate tracking
   */
  async createGroupAffiliateLink(userId: number, dto: CreateGroupAffiliateLinkDto) {
    console.log(`[DEBUG] Creating group affiliate link for user ${userId}:`, dto);
    
    // 1. Validate user and enable affiliate status
    const user = await this.userRepo.findOne({ where: { id: userId } });
    if (!user) {
      throw new NotFoundException('User not found');
    }
    
    console.log(`[DEBUG] User found: ${user.email}, is_affiliate: ${(user as any).is_affiliate}`);
    
    // Auto-enable affiliate status if not already enabled
    if (!(user as any).is_affiliate || !(user as any).code) {
      const affiliateCode = (user as any).code || `AFF${userId}${Date.now().toString().slice(-4)}`;
      await this.userRepo.update(userId, {
        is_affiliate: true,
    ...
    if (dto.programId) {
      ...
      if (program.status !== 'active') {
        throw new ForbiddenException('Affiliate program is not active');
      }
    }

    // 4. Create group order with affiliate tracking
    const now = new Date();
    const expiresAt = dto.expiresAt ? new Date(dto.expiresAt) : null;
    
    if (expiresAt && expiresAt <= now) {
      throw new ForbiddenException('Expiration date must be in the future');
    }

    // Generate unique join code
    const joinCode = this.generateJoinCode();
    const groupName = dto.groupName || `Group by ${user.email}`;

    const groupOrder = this.groupOrderRepo.create({
      store: { id: dto.storeId } as any,
      user: { id: userId } as any, // host
      name: groupName,
      status: 'open',
      expires_at: expiresAt,
      join_code: joinCode,
      target_member_count: dto.targetMemberCount || 2,
      // Host-based affiliate tracking
      group_affiliate_code: (user as any).code,
      group_affiliate_user_id: userId,
      group_affiliate_program_id: dto.programId || null,
      affiliate_detection_method: 'host_inheritance',
      affiliate_detected_at: now,
    });

    const savedGroup = await this.groupOrderRepo.save(groupOrder);
    
    // 5. Update invite link with UUID
    const baseUrl = process.env.FE_BASE_URL || 'http://localhost:5173';
    const inviteLink = `${baseUrl}/group/${savedGroup.uuid}`;
    
    await this.groupOrderRepo.update(savedGroup.id, { 
      invite_link: inviteLink 
    });

    // 6. Create affiliate link record for tracking
    const affiliateLinkCode = `GROUP:${userId}:${savedGroup.id}`;
    
    const affiliateLink = this.linkRepo.create({
      user_id: user,
      program_id: program || undefined,
      code: affiliateLinkCode,
      created_at: new Date(),
    });

    const savedLink = await this.linkRepo.save(affiliateLink);

    console.log(`✅ Created group affiliate link - Group: ${savedGroup.id}, Link: ${savedLink.id}`);

    return {
      group_id: savedGroup.id,
      group_uuid: savedGroup.uuid,
      link_id: savedLink.id,
      invite_link: inviteLink,
      join_code: joinCode,
      affiliate_code: (user as any).code,
  ...
  /**
   * Get user's group affiliate links
   */
  async getMyGroupAffiliateLinks(userId: number) {
    const groups = await this.groupOrderRepo.find({
      where: { 
        user: { id: userId } as any,
        group_affiliate_user_id: userId // Only groups where user is the affiliate
      },
      relations: ['store'],
      order: { created_at: 'DESC' },
    });
    ...
    const mapped = groups.map((group) => ({
      ...
      affiliate_code: group.group_affiliate_code,
      program_id: group.group_affiliate_program_id,
      store_name: (group.store as any)?.name,
  ...
  /**
   * Generate random join code for group
   */
