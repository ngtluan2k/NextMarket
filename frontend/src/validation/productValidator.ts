    ...
    if (!Array.isArray(form?.variants) || form.variants.length === 0) {
      ...
      form.variants.forEach((v: any, i: number) => {
        if (!isNonEmptyString(v?.sku))
          E(errors, ['variants', i, 'sku'], 'SKU biến thể là bắt buộc');
        if (!isNonEmptyString(v?.variant_name))
          E(
            errors,
            ['variants', i, 'variant_name'],
            'Tên biến thể là bắt buộc'
          );
    ...
    inv.forEach((it: any, i: number) => {
      if (!isNonEmptyString(it?.variant_sku))
        E(
          ...
          'SKU biến thể (variant_sku) là bắt buộc'
        );
      if (!isNonEmptyString(it?.location))
        E(
          errors,
          ['inventory', i, 'location'],
          'Vị trí/kho (location) là bắt buộc'
        );
    ...
    variants.forEach((v: any) => {
      ...
      if (sum <= 0) {
        E(
          errors,
          ['inventory'],
          `Biến thể "${v?.sku || '(chưa có SKU)'}" phải có tổng tồn kho > 0`
        );
      }
    });
    inv.forEach((i: any) => {
      if (!skuSet.has(i?.variant_sku)) {
        E(
          errors,
          ['inventory'],
          `Hàng tồn kho có SKU "${i?.variant_sku}" không khớp với bất kỳ biến thể nào`
        );
      }
    });
