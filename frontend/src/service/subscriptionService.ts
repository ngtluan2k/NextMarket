...
import { Subscription, UseSubscriptionBody, UseSubscriptionResponse } from '../app/types/subscription';
...
export async function useSubscriptionApi(body: UseSubscriptionBody): Promise<UseSubscriptionResponse> {
  const token = localStorage.getItem('token');
