...
import {
  Mail, Lock, User, Calendar, Phone, BadgeCheck, Eye, EyeOff,
} from "lucide-react";
...
export default function LoginModal({
  ...
  const handleLogin = async (e?: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      if (onLogin) await onLogin({ email, password, remember });
      else await callDefaultLogin({ email, password, remember });
      onClose();
    } catch (err: any) {
  ...
  const handleRegister = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // chuyển sang login & điền sẵn email
      setMode("login");
      setEmail(reg.email);
      setPassword("");
    } catch (err: any) {
