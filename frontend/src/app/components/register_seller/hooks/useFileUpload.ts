...
export const useFileUpload = () => {
  ...
  const uploadCCCD = async (
    storeId: number | null,
    onSuccess: (frontUrl: string, backUrl: string) => void
  ) => {
    ...
    if (storeId) {
      ...
      if (cccdFrontFile) {
        ...
        if (!res.ok) {
          ...
          throw new Error(
            errorData?.message || 'Upload CCCD mặt trước thất bại'
          );
        }
        const data = await res.json();
    ...
    if (cccdFrontFile) {
      ...
      const data = await res.json();
      if (!res.ok)
        throw new Error(data?.message || 'Upload tạm mặt trước thất bại');
    ...
    if (cccdBackFile) {
      ...
      const data = await res.json();
      if (!res.ok)
        throw new Error(data?.message || 'Upload tạm mặt sau thất bại');
