...
export const validateStep1 = (formData: SellerFormData, addresses: any[]) => {
  ...
  if (!defaultAddress) {
    ...
    const requiredFields = ['recipient_name', 'phone', 'street', 'province', 'postal_code'];
    const missingFields = requiredFields.filter(field => !(defaultAddress as any)[field]?.trim());
...
export const validateStep3 = (formData: SellerFormData) => {
  ...
  if (formData.bank_account?.bank_name || formData.bank_account?.account_number || formData.bank_account?.account_holder) {
  ...
  if (formData.store_identification?.full_name || formData.store_identification?.img_front || formData.store_identification?.img_back) {
