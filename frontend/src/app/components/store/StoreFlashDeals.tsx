...
type Props = {
  ...
  fetchFlashDeals?: (slug: string) => Promise<{ items: FlashDealItem[]; endsAt?: string }>;
  className?: string;
};

export default function StoreFlashDeals({ storeSlug, endsAt, fetchFlashDeals, className }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const data = await (fetchFlashDeals ?? defaultFetchFlashDeals)(storeSlug);
        if (!alive) return;
        setItems(data.items || []);
  ...
  return (
    <section className={["mt-3 rounded-2xl border border-slate-200 bg-white shadow-sm", className || ""].join(" ")}>
      {/* header */}
      <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3 sm:px-6">
        <div className="flex items-center gap-3">
          <h2 className="text-base font-semibold text-rose-600">Giá Sốc ⚡ Hôm Nay</h2>
          {clock && (
            <div className="flex items-center gap-1 text-[13px] font-semibold text-rose-600">
              {clock.split(":").map((seg, i) => (
                <span key={i} className="rounded-md bg-rose-50 px-1.5 py-0.5">{seg.trim()}</span>
              ))}
            </div>
          )}
        </div>
        <Link to={`/store/${storeSlug}/deals`} className="text-sm font-medium text-blue-600 hover:underline">
          Xem tất cả
        </Link>
      </div>
        ...
        >
          {loading ? (
            Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="h-56 w-[200px] shrink-0 overflow-hidden rounded-lg border border-slate-200">
            ...
            items.map((p) => (
              ...
              >
                ...
                <div className="mt-2 line-clamp-2 h-[2.6rem] text-sm text-slate-800">{p.name}</div>
                ...
                <div className="mt-2">
                  <span className="inline-flex items-center gap-1 rounded-full bg-rose-100 px-3 py-1 text-[11px] font-medium text-rose-600">
                    <span className="h-2 w-2 rounded-full bg-rose-500" /> Vừa mở bán
                  </span>
                </div>
              </div>
            ))
...
async function defaultFetchFlashDeals(slug: string): Promise<{ items: FlashDealItem[]; endsAt?: string }> {
  const res = await fetch(`/api/stores/${encodeURIComponent(slug)}/flash-deals`);
  const ct = res.headers.get("content-type") || "";
  if (!res.ok) {
    const t = await res.text(); throw new Error(`HTTP ${res.status}: ${t.slice(0, 120)}`);
  }
  if (!ct.includes("application/json")) {
    const t = await res.text(); throw new Error("API không trả JSON: " + t.slice(0, 120));
  }
  return res.json();
}
