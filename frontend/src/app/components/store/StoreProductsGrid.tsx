...
type Props = {
  ...
  fetchProducts?: (args: {
    slug: string; q: string; sort: SortKey; page: number; pageSize: number;
...
export default function StoreProductsGrid({
    ...
    (async () => {
      ...
      try {
        const res = await (fetchProducts ?? defaultFetchProducts)({
          slug: storeSlug, q, sort, page, pageSize,
        });
  ...
  const totalPages = useMemo(
    () => Math.max(1, Math.ceil((data?.total || 0) / pageSize)), [data?.total, pageSize]
  );
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5">
            {data.items.map(p => (
              <Link
                to={`/product/${p.id}`}
                key={p.id}
                className="group overflow-hidden rounded-lg border border-slate-200 bg-white transition hover:-translate-y-[1px] hover:shadow-md"
              >
...
/** Fetch mặc định -> /api/stores/:slug/products?q=&sort=&page=&pageSize= */
async function defaultFetchProducts({
  slug, q, sort, page, pageSize,
}: { slug: string; q: string; sort: SortKey; page: number; pageSize: number; }): Promise<StoreProductsResponse> {
  const params = new URLSearchParams({ q, sort, page: String(page), pageSize: String(pageSize) });
  const res = await fetch(`/api/stores/${encodeURIComponent(slug)}/products?` + params.toString());
