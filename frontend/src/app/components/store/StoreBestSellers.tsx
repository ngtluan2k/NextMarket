...
export default function StoreBestSellers({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const data = await (fetchBestSellers ?? defaultFetchBestSellers)(
          storeSlug
        );
  ...
  return (
    <section className="mt-3 rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3 sm:px-6">
        ...
        <Link
          to={`/store/${storeSlug}/bestsellers`}
...
async function defaultFetchBestSellers(
  slug: string
): Promise<BestSellerProduct[]> {
  const res = await fetch(
    `/api/stores/${encodeURIComponent(slug)}/best-sellers`,
    {}
  );
