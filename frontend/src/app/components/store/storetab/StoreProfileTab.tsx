...
async function defaultFetchProfile(slug: string): Promise<StoreProfileData> {
  const res = await fetch(`/api/stores/${encodeURIComponent(slug)}/profile`, {
    headers: { Accept: "application/json" },
  });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  const ct = res.headers.get("content-type") || "";
  if (!ct.includes("application/json")) throw new Error("API không trả JSON");
  return res.json();
}
...
export default function StoreProfileTab({ data, fetchProfile, className }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const d = await (fetchProfile ?? defaultFetchProfile)(slug);
        if (alive) setInfo(d);
      } finally {
  ...
  return (
    <section className={["mx-auto mt-3 max-w-[1300px]", className || ""].join(" ")}>
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="px-6 py-6">
          {/* ✅ Grid chia 400px (trái) + auto (phải) */}
          <div className="grid grid-cols-1 md:grid-cols-[400px_auto] gap-6">
            ...
            <div className="flex items-center justify-center">
              <div className="grid grid-cols-[1fr_auto_1fr] w-full text-center">
                <div>
                  ...
                  <div className="text-[36px] font-semibold leading-none text-emerald-600">{cancelRate} %</div>
                </div>
                ...
                <div>
                  ...
                  <div className="text-[36px] font-semibold leading-none text-emerald-600">{returnRate} %</div>
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-slate-200 md:border-t-0 md:border-l">
              <Row icon={Icon.calendar} label="Thành viên từ năm">{info?.memberSinceYear ?? "—"}</Row>
              <Row icon={Icon.box} label="Sản phẩm">
                {typeof info?.totalProducts === "number" ? `${fmt.format(info.totalProducts)}+` : "—"}
              </Row>
              ...
              <Row icon={Icon.starStroke} label="Đánh giá">
                {typeof info?.rating === "number"
                  ? <>{info.rating.toFixed(1)} / 5 {info?.ratingCountText && <span className="text-slate-500 text-sm">({info.ratingCountText})</span>}</>
                  : "—"}
              </Row>
              <Row icon={Icon.users} label="Người theo dõi">
                {typeof info?.followers === "number" ? `${fmt.format(info.followers)}+` : "—"}
              </Row>
              <Row icon={Icon.chat} label="Phản hồi Chat">{info?.chatResponse || "—"}</Row>
            </div>
