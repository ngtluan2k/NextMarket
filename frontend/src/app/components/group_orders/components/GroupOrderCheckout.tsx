...
export const GroupOrderCheckout: React.FC<GroupOrderCheckout> = ({
    ...
    try {
      ...
      if (deliveryMode === 'host_address') {
        ...
        const defaultAddress =
          addressesData.find((a: UserAddress) => a.isDefault) ||
          addressesData[0];
        if (defaultAddress) {
  ...
  console.log('🧩 groupItems trước khi navigate:', groupItems);
  ...
  const totals = React.useMemo(() => {
    const items =
      Array.isArray(groupItems) && groupItems.length > 0 ? groupItems : [];
  ...
  return (
    <>
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500">
                      {item.member?.user?.profile?.full_name ||
                  ...
                  <div className="text-right">
                    <p className="font-medium">
                      {formatPrice(getItemPreGroupPrice(item, discountPercent))}
                    </p>
                  </div>
                </div>
              ))}
      ...
      <AddressModal
        visible={showAddressModal} // chỉ dùng state
