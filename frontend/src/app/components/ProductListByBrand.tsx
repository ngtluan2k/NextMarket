...
const ProductListByBrand: React.FC<Props> = ({ brandIds, categoryIds, title }) => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        for (const id of brandIds) {
          const res = await fetch(`http://localhost:3000/brands/${id}/products`);
        ...
        const filtered = categoryIds && categoryIds.length > 0
          ? allProducts.filter((p) =>
              p.categories?.some((c) => c.category && categoryIds.includes(c.category.id))
            )
          : allProducts;
