...
export default function AddressForm({
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoadingD(true);
      try {
        setDistricts(await getDistricts(v.provinceCode!));
      } finally {
        setLoadingD(false);
      }
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoadingW(true);
      try {
        setWards(await getWards(v.districtCode!));
      } finally {
        setLoadingW(false);
      }
    })();
  ...
  const onProvinceChange = (codeStr: string) => {
    ...
    set('province', p?.name || '');
    set('districtCode', undefined as any);
    set('district', '');
    set('wardCode', undefined as any);
    set('ward', '');
  };
  const onDistrictChange = (codeStr: string) => {
    const code = Number(codeStr) || undefined;
    const d = districts.find((x) => x.code === code);
    set('districtCode', code);
    set('district', d?.name || '');
    set('wardCode', undefined as any);
    set('ward', '');
  };
  ...
  const S = {
    xs: {
      label: 'text-[13.5px]',
      ctrl: 'h-9 text-[13.5px] px-3',
      area: 'h-24 text-[13.5px] px-3',
      gap: 'gap-2',
    },
    sm: {
      label: 'text-sm',
      ctrl: 'h-10 text-sm px-3',
      area: 'h-28 text-sm px-3',
      gap: 'gap-3',
    },
    md: {
      label: 'text-base',
      ctrl: 'py-2 text-base px-3',
      area: 'h-32 text-base px-3',
      gap: 'gap-4',
    },
  }[size];
  ...
  return (
    <form
      className={`grid grid-cols-1 ${S.gap}`}
      onSubmit={async (e) => {
        e.preventDefault();
        await onSubmit(v);
      }}
    >
      ...
      <div className="mt-1 flex gap-3">
        <button
          type="submit"
          className={`rounded-md bg-yellow-400 px-4 ${
            size === 'xs' ? 'h-9 text-sm' : 'py-2'
