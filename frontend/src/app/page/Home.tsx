...
function BootstrapTwoUpCarousel({
  ...
  const pages = useMemo(() => {
    const out: Slide[][] = [];
    for (let i = 0; i < normalized.length; i += 2) out.push(normalized.slice(i, i + 2));
    return out;
  }, [normalized]);
              ...
              <div className="row g-3">
                {group.map((s, j) => (
                  <div key={j} className="col-12 col-lg-6">
                    <a href={s.href || "#"} aria-label={s.alt || `banner ${idx * 2 + j + 1}`}>
                      <img
                        className="w-100"
                        style={{
                          aspectRatio: "16 / 9",
...
const Home: React.FC = () => {
  const [slidesState, setSlidesState] = useState<Slide[] | undefined>(undefined);
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      ...
      <main className="mx-auto max-w-screen-2xl px-4 pb-10">
        <div className="grid grid-cols-1 lg:grid-cols-[288px_minmax(0,1fr)] gap-6 mt-4">
          {/* Danh mục bên trái */}
          ...
          <section>
            ...
            <FeaturedBrands className="mt-4" />
            <YouMayAlsoLike className="mt-6"/>
            <FlashSale className="mt-4" />

            <ProductGridToday className="mt-4" />
