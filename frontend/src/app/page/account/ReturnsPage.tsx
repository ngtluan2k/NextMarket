...
export default function ReturnsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const data = (await res.json()) as { items: ReturnSummary[]; hasMore?: boolean };

        if (!cancelled) {
          setRows(page === 1 ? data.items ?? [] : (prev) => [...prev, ...(data.items ?? [])]);
  ...
  const statusPill = (s: ReturnTab) => {
    ...
    if (s === "in_progress") return (
      <span className={`${base} bg-sky-50 text-sky-700`}>
        <RotateCcw className="h-3 w-3" />Đang tiến hành
      </span>
    );
    if (s === "done") return (
  ...
  return (
    <>
      <h1 className="text-2xl font-semibold text-slate-900 mb-4">Quản lý đổi trả</h1>
              ...
              return (
                <button
                  ...
                  className={`px-3 py-2 text-sm rounded-t-md ${
                    active ? "text-sky-700 border-b-2 border-sky-600" : "text-slate-600 hover:text-slate-900"
                  }`}
                  aria-pressed={active}
                >
                  {t.label}
        ...
        <div className="p-3">
              ...
              {Array.from({ length: 3 }).map((_, i) => (
                <li key={i} className="rounded-xl border border-slate-200 p-4">
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-3">
                    {[0,1,2].map(k => <div key={k} className="h-16 bg-slate-100 rounded animate-pulse" />)}
                  </div>
                </li>
              ))}
            </ul>
                  ...
                  <li key={r.id} className="rounded-xl border border-slate-200 p-4">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="text-sm text-slate-700">
                        <span className="font-medium">Mã yêu cầu:</span> {r.code}
                        {r.orderCode ? <span className="ml-3 text-slate-500">Đơn: {r.orderCode}</span> : null}
                        {r.createdAt ? <span className="ml-3 text-slate-500">{formatTime(r.createdAt)}</span> : null}
                      </div>
                      {statusPill(r.status)}
                    </div>
                    ...
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-3">
                      <div className="text-sm text-slate-600 line-clamp-1">
                        {r.reason ? <>Lý do: <span className="text-slate-900">{r.reason}</span></> : <>&nbsp;</>}
                      </div>
                      <div className="flex items-center gap-2">
                        <a href={`/account/returns/${r.id}`} className="rounded-lg border border-slate-200 px-3 py-2 text-sm hover:bg-slate-50">
                          Chi tiết
                        </a>
                        {r.status === "in_progress" && (
                          <a href={`/account/returns/${r.id}#messages`} className="rounded-lg bg-sky-600 px-3 py-2 text-sm font-medium text-white hover:bg-sky-700">
                            Trao đổi
                          </a>
                        )}
                      </div>
          ...
          {!loading && rows.length === 0 && (
            <div className="grid place-items-center py-14 text-center">
              ...
              <a href="/" className="mt-4 inline-block rounded-lg bg-amber-400 px-4 py-2 text-sm font-medium text-white hover:bg-amber-500">
                Tiếp tục mua sắm
              </a>
            </div>
          )}
