  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/stores/${storeId}/upload-logo`,
        {
          method: 'POST',
          headers: {
...
export default function StoreInfoTab() {
  ...
  const refresh = async () => {
    try {
      ...
      console.error(err);
      message.error(
        err?.response?.data?.message || 'Không tải được thông tin cửa hàng'
      );
    } finally {
      setLoading(false);
    }
      ...
      <Card title={`Cửa hàng ${store?.id} - ${store?.name ?? '-'}`}>
            ...
            <div className="flex-1">
              <Descriptions bordered column={2} size="middle">
                ...
                <Descriptions.Item label="Cấp độ">
                  <Tag
                    color={
                      storeLevel?.level === 'premium'
                        ? 'gold'
                        : storeLevel?.level === 'trusted'
                        ? 'blue'
                        : 'green'
                    }
                  >
                ...
                <Descriptions.Item label="Nâng cấp lúc">
                  {storeLevel?.upgraded_at
                    ? new Date(storeLevel.upgraded_at).toLocaleString()
                    : '-'}
                </Descriptions.Item>
                ...
                <Descriptions.Item label="Rating TB">
                  {rating?.average?.toFixed?.(2) ?? 0}
                </Descriptions.Item>
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {!bizEdit ? (
            <Button
              type="primary"
              icon={<EditOutlined />}
              onClick={startBizEdit}
            >
              Sửa
            </Button>
          ) : (
            <Space>
              <Button size="small" onClick={() => setBizEdit(false)}>
                Hủy
              </Button>
              <Button size="small" type="primary" onClick={saveBiz}>
                Lưu
              </Button>
            </Space>
          )}
        ...
        {!bizEdit ? (
          <Descriptions bordered column={2} size="small">
            ...
            <Descriptions.Item label="Tên pháp lý">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {!idenEdit ? (
            <Button
              type="primary"
              icon={<EditOutlined />}
              onClick={startIdenEdit}
            >
              Sửa
            </Button>
          ) : (
            <Space>
              <Button size="small" onClick={() => setIdenEdit(false)}>
                Hủy
              </Button>
              <Button size="small" type="primary" onClick={saveIden}>
                Lưu
              </Button>
            </Space>
          )}
        ...
        {!idenEdit ? (
          ...
          <Form form={idenForm} layout="vertical">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Form.Item name="img_front" label="URL ảnh trước">
                <Input />
              </Form.Item>
              <Form.Item name="img_back" label="URL ảnh sau">
                <Input />
              </Form.Item>
            </div>
          </Form>
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {!addrEdit ? (
            <Button
              type="primary"
              icon={<EditOutlined />}
              onClick={startAddrEdit}
            >
              Sửa
            </Button>
          ) : (
            <Space>
              <Button size="small" onClick={() => setAddrEdit(false)}>
                Hủy
              </Button>
              <Button size="small" type="primary" onClick={saveAddr}>
                Lưu
              </Button>
            </Space>
          )}
        ...
        {!addrEdit ? (
          ...
          <Form form={addrForm} layout="vertical">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Form.Item name="recipient_name" label="Người nhận">
                <Input />
              </Form.Item>
              <Form.Item name="phone" label="SĐT">
                <Input />
              ...
              <Form.Item name="district" label="Huyện/Xã">
                <Input />
              </Form.Item>
              ...
              <Form.Item
                ...
                label="Loại địa chỉ"
                rules={[
                  { required: true, message: 'Nhập loại (vd: warehouse)' },
                ]}
              >
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {!bankEdit ? (
            ...
            <Space>
              <Button size="small" onClick={() => setBankEdit(false)}>
                Hủy
              </Button>
              <Button size="small" type="primary" onClick={saveBank}>
                Lưu
              </Button>
            </Space>
          )}
        ...
        {!bankEdit ? (
          <Descriptions title="" bordered column={2} size="small">
            ...
            <Descriptions.Item label="Số TK" span={2}>
          ...
          <Form form={bankForm} layout="vertical">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Form.Item name="account_holder" label="Chủ TK">
                <Input />
              </Form.Item>
              <Form.Item
                className="md:col-span-2"
