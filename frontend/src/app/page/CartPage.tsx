...
const CartPage: React.FC<CartProps> = ({ showMessage }) => {
  ...
  const [groupFilter, setGroupFilter] = useState<"all" | "host" | "expiring">("all");
  ...
  const loadActiveGroups = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/group-orders/user/${user.id}/active`,
        {
            ...
            <Collapse bordered={false} defaultActiveKey={["groups"]}>
              ...
              >
                ...
                {filteredGroups.length === 0 ? (
                  <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="Không có nhóm phù hợp" />
                ) : groupView === "grid" ? (
                        ...
                        <div className="flex items-start justify-between gap-10">
                          <div>
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              {group.status === "CLOSING" && <Tag color="red">Sắp đóng</Tag>}
                            ...
                            <div className="text-[11px] text-gray-500 mt-1">
                              Tham gia:{" "}
                              {new Date(group.joined_at).toLocaleString("vi-VN")}
                            </div>
                            {group.expires_at && (
                  ...
                  <Space direction="vertical" size="small" style={{ width: "100%" }}>
                    {filteredGroups.map((group) => (
                      <Card key={group.id} size="small" className="border-blue-200">
      ...
      <div className="mb-6">
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-gray-900 font-semibold">Sản phẩm mua chung</span>
            ...
            <div className="ml-auto">
              ...
              >
                {expanded ? "Thu gọn" : rest > 0 ? `Xem tất cả (+${rest})` : "Ẩn/Hiện"}
              </Button>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {!expanded && rest > 0 && <Tag color="purple">+{rest} SP khác…</Tag>}
          </div>

          <div className="text-xs text-gray-600 mt-2">
            Các sản phẩm này sẽ được thanh toán trong đơn hàng nhóm, không gộp với
            sản phẩm thường.
          </div>
        </Card>
      </div>
      ...
      <main className="mx-auto w-full max-w-[1500px] px-4 lg:px-6 py-6 flex-1">
        ...
        {cart.length === 0 ? (
          <Card className="text-center border-0 shadow-sm rounded-xl">
            <Empty description="Giỏ hàng trống" style={{ marginTop: 48, marginBottom: 48 }} />
            <Button type="primary" size="large" onClick={() => navigate("/stores")} className="rounded-lg">
              Tiếp tục mua sắm
            </Button>
          </Card>
        ) : (
